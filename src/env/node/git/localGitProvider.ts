		...
		async function getCommitsForGraphCore(
			...
			for (const commit of commits) {
				...
				rows.push({
					...
					author: isCurrentUser ? '我' : commit.author,
					email: commit.authorEmail,
			...
			for (const s of stashes) {
				...
				commits.set(
					...
					new GitCommit(
						...
						repoPath,
						s.sha,
						new GitCommitIdentity('我', undefined, new Date((s.date as any) * 1000)),
						new GitCommitIdentity('我', undefined, new Date((s.committedDate as any) * 1000)),
						message.split('\n', 1)[0] ?? '',
						s.parents.split(' '),
						message,
						s.files?.map(
